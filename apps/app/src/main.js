            ...
            setTimeout(() => {
                ...
                if (char !== ' ') {
                    try {
                        ...
                        sound.volume = 0.3;
                        sound.play().catch(() => {
                            // Ignore audio errors in case audio is not supported
                        });
                    } catch (e) {
                        // Ignore audio errors
                    }
    ...
    generateNewFortune() {
        if (typeof PluginMessageHandler !== 'undefined') {
            PluginMessageHandler.postMessage(JSON.stringify({
                message: "Generate a dystopian fortune cookie message. 3 to 5 words only. Make it weird and eerily applicable to today's AI-overrun world. Just respond with the fortune text, nothing else.",
                useLLM: true,
    ...
    async saveFortune(fortune) {
        ...
        try {
            if (window.creationStorage && window.creationStorage.plain) {
                ...
                await window.creationStorage.plain.setItem('fortune_data', btoa(data));
            }
        } catch (error) {
            console.error('Storage error:', error);
        }
